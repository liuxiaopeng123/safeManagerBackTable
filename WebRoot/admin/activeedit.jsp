...
%>
<form action="/qgzxmis/mainCtrl?ac=activeedit&id=<%=id %>" name="f1" method="post" onsubmit="return check()"  enctype="multipart/form-data">
    ...
    <tr>
       ...
       <tr>
        ...
        <select id="type" name="type">
        	...
        	<option value="<%=map1.get("id") %>" <%if(map1.get("id").equals(map.get("type"))){out.print("selected==selected");} %>><%=map1.get("typename") %></option>
        	<%} %>
        </select>
        </label></td>
       </tr>
       <tr>
        ...
        <TD bgcolor="#FFFFFF" class="STYLE19" width="68%" height=24><label>
        <input name="jt" type="text" id="jt" size="5" onkeyup='checkDecimal(this)' value="<%=map.get("jt") %>"/>元/小时
        </label></td>
       </tr>
       <tr>
        <TD width="20%" height=31 align="center" bgcolor="#FFFFFF" class="STYLE19">截止时间</td>
        <TD bgcolor="#FFFFFF" class="STYLE19" width="68%" height=24><label>
        <input name="etime" type="text" id="etime"  value="<%=map.get("etime") %>" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"/>
