...
<BODY>
<form action="xskaoq.jsp?f=f" name="f1" method="post">

<%
String url="xskaoq.jsp?1=1";
String key = request.getParameter("key")==null?"":request.getParameter("key");
String sql = "select * from joininfo where overtime is  null ";
if(!key.equals(""))
{
url+="&key="+key;
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce" >
  <TBODY>
  <TR>
    <TD colSpan=2 height=24 bgcolor="#353c44"><span class="STYLE1">学生考勤管理</span></TD> 
  </TR>
  </TBODY></TABLE>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce" >
  ...
  <TR>
    ...
    <TD  align="center" bgcolor="#d3eaef" class="STYLE10">考勤</TD>
  </TR>
  <%
  ...
  PageManager pageManager = PageManager.getPage(url,10, request);
	    ...
	    for(HashMap m:nlist)
	    {
	    	HashMap sm = dao.select("select * from sysuser where id='"+m.get("studentid")+"'").get(0);
	    	HashMap gm = dao.select("select * from active where id='"+m.get("stid")+"'").get(0);
  ...
  <TR>
    ...
    <TD  height=24 align="center" bgcolor="#FFFFFF" class="STYLE19"><a href="javascript: update('<%=m.get("id") %>')">录入</a></TD>
  </TR>
  <%} %>
