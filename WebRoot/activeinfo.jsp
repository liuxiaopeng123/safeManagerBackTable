                          ...
                          <TR vAlign=top align=middle>
                            <TD width="300%" colSpan=3>
                              ...
                              cellPadding=0 width="100%">
                                ...
                                String sql = "select  * from active where st in (select stid from joininfo where studentid='"+user.get("id")+"' and shstatus='通过') order by savetime desc "; 
                                
                                String url ="/qgzxmis/activeinfo.jsp?1=1";
							    ...
							    {%>
                                <TR>
                                <TD class=dd height=30 vAlign=center width="50%" 
                                align=left>· <A title=<%=m.get("title") %> 
                                href="activeinfox.jsp?id=<%=m.get("id") %>" 
                                target=""><%=m.get("title") %></A>
                                 </TD>
                                <TD class=dd height=30 vAlign=center width="29%" 
                                align=right><SPAN>
                                <%HashMap mmm = dao.select("select * from st where id="+m.get("st")).get(0); %> 
                              		【<%=mmm.get("pname") %>】
